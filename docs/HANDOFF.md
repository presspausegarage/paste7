...
- **What**: Tauri 2 + React + Monaco desktop scratchpad. Two workflows ship today: Scratchpad (paste-and-redact for HL7 v2 / v3 / CDA / FHIR) and DICOM SR (file-drop SR-header redaction; SR-only after the 2026-05-01 rescope). HL7-viewer OCR (image paste/drop → Windows.Media.Ocr → HL7 normalization → existing HL7 v2 walker) is planned for Phase 6.
...
- **Update (2026-07-18)**: **v0.1.0 released publicly** from `main` commit `12dc645`. The GitHub release contains `paste7_0.1.0_x64-setup.exe` (3.9 MB) and its `.sha256` checksum; release workflow gates passed. The IP/moonlighting gate was cleared July 15 after the People Team confirmed no IP-assignment, confidentiality, non-compete, or moonlighting agreement exists in Andy's employee file.
- **Next**: Tauri updater wiring (`tauri signer generate` for the updater keypair, `plugins.updater` config pointing at the GitHub Releases endpoint, `updater:default` capability grant). This is the only remaining Phase 5 item; it is not blocked, just unscheduled.
...
All six steps shipped (see Phase 2 commit table). Engine wired, split Monaco panes, findings panel with filters, tokenized tree, format chrome, copy guards, in-memory lint. **First-launch smoke test passed 2026-07-15**: `npm run dev` compiled the Rust shell and Vite frontend cleanly, the app launched, and the process remained stable. See **Phase 2 polish backlog** below for items the rough-order spec scoped down.
